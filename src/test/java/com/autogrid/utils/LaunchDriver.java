...
public class LaunchDriver {
	...
	private LaunchDriver() throws MalformedURLException {
		...
		if (Config.get("browser").equalsIgnoreCase("chrome")
			...
			URL gridUrl = new URL(
					String.format(Config.get("selenium.grid.urlFormat"), Config.get("selenium.grid.hubHost")));
			...
			ChromeOptions options = new ChromeOptions();
			options.addArguments(Arrays.asList("--no-sandbox", "--verbose", "--ignore-certificate-errors",
	...
	private static java.util.Map<String, Object> createMobileEmulation() {
		// Create a Map for mobile emulation (you can use predefined devices or custom
		// configurations)
