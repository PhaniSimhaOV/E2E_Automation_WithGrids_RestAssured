...
public class DatabaseConnectionStepDefinition {
	...
	private String filePath = "src/test/resources/config/output.xlsx"; // Output Excel file path
	...
	public void user_tries_to_save_the_fetched_data_into_the_file() {
		try {
			...
			List<String> queries = Arrays.asList("SELECT \r\n"
					...
					+ "LEFT JOIN dms_onroad_price dop ON dop.lead_id = dl.id\r\n"
					+ "where dmls.lead_stage_key='INVOICE' and dl.organization_id=16 LIMIT 3;",
					...
					+ "where dmls.lead_stage_key='DELIVERY' and dl.organization_id=16 LIMIT 3\r\n","select \r\n"
							+ "x.model,x.variant,x.fuel_type,x.vin_number,x.transmission_type,x.test_drive_datetime,x.reason as followUpType,x.customer_remarks as followupRemarks\r\n"
							+ "from\r\n"
							+ "test_drive_history_details  x \r\n"
							+ "where \r\n"
							+ "customer_id in (select crm_universal_id from dms_lead dl where  dl.organization_id=16) order by test_drive_datetime desc LIMIT 3;",
							"SELECT \r\n"
							+ "    da.vinno,\r\n"
							+ "    dd.warrantyamount,\r\n"
