...
public class EnquiryParentStepDefinition {
    ...
    public void CreateTestDriveAndAddFollowup() throws InterruptedException, IOException {
        ...
        // Add a new column for error logging
        ExcelWriting.addColumnToSheet(filePath, sheetName, "Error Logs");
        Runtime.getRuntime().exec("adb shell am broadcast -a android.intent.action.MAIN --ez \"extra_is_slow\" false");
            ...
            try {
                ...
                try {
                    if (enquiryParentSteps.getDrpDownTestDriver().isDisplayed()) {
                        ...
                        commonActions.scrollDown();
                        Thread.sleep(3000);
                        for (WebElement slot : enquiryParentSteps.getTimeSlots()) {
                        ...
                        if (value) {
                            commonActions.clickElement(enquiryParentSteps.getTestDriveDate());
                            int count = 0;
                            for (WebElement slot : enquiryParentSteps.getDateSlots()) {
                                if (slot.getDomAttribute("clickable").equalsIgnoreCase("true")
                                        && slot.getDomAttribute("enabled").equalsIgnoreCase("true")) {
                                    count++;
                                    if (count == 2) {
                                        commonActions.clickElement(slot);
                                        commonActions.clickElement(enquiryParentSteps.getCalenderOkButton());
                                        break;
                                    }
                                }
                            }
            ...
            try {
                ...
                try {
                    ...
                    try {
                        ...
                        System.out.println("Loader was not there...");
                    }
                }
